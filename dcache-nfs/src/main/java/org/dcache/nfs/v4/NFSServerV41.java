...
public class NFSServerV41 extends nfs4_prot_NFS4_PROGRAM_ServerStub {
	...
	private static final RequestExecutionTimeGauges<String> GAUGES = new RequestExecutionTimeGauges<>(
			NFSServerV41.class.getName());

	public NFSServerV41(NFSv4OperationFactory operationFactory,
			NFSv41DeviceManager deviceManager, VirtualFileSystem fs,
			NfsIdMapping idMapping, ExportFile exportFile)
		...
		try {
			...
			if (minorversion > 1) {
				throw new MinorVersMismatchException(String.format(
						"Unsupported minor version [%d]",
						arg1.minorversion.value));
			}

			if (arg1.argarray.length >= NFSv4Defaults.NFS4_MAX_OPS
					&& minorversion == 0) {
				/*
			...
			CompoundContext context = new CompoundContext(
					arg1.minorversion.value, fs, _statHandler, _deviceManager,
					call$, _idMapping, _exportFile, arg1.argarray.length);
				...
				try {
					if (minorversion != 0) {
						...
						if (position == 1) {
							 ...
							 */
							if (arg1.argarray.length > context.getSession()
									.getMaxOps()) {
								throw new TooManyOpsException(String.format(
										"Too many ops [%d]",
										arg1.argarray.length));
							}
							...
							if (cache != null) {
								res.resarray.addAll(cache.subList(position,
										cache.size()));
					...
					_log.error("NFS server run out of resources: {}",
							e.getMessage());
			...
			if (!retransmit && context.cacheThis()) {
				context.getSession().updateSlotCache(context.getSlotId(),
						res.resarray);
			}
	 ...
	 * from NFSv4.1 spec:
	 * 
	 * SEQUENCE MUST appear as the first operation of any COMPOUND in which it
	 * appears. The error NFS4ERR_SEQUENCE_POS will be returned when it is found
	 * in any position in a COMPOUND beyond the first. Operations other than
	 * SEQUENCE, BIND_CONN_TO_SESSION, EXCHANGE_ID, CREATE_SESSION, and
	 * DESTROY_SESSION, MUST NOT appear as the first operation in a COMPOUND.
	 * Such operations MUST yield the error NFS4ERR_OP_NOT_IN_SESSION if they do
	 * appear at the start of a COMPOUND.
	 */
	private static void checkOpPosition(int opCode, int position, int total)
			throws ChimeraNFSException {

		/*
		 * special case of illegal operations.
		 */
		if (opCode > nfs_opnum4.OP_RECLAIM_COMPLETE
				|| opCode < nfs_opnum4.OP_ACCESS) {
			return;
		}
