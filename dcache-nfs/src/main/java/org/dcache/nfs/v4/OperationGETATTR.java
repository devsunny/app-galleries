...
public class OperationGETATTR extends AbstractNFSv4Operation {
	...
	private static final Logger _log = LoggerFactory
			.getLogger(OperationGETATTR.class);
	...
	public void process(CompoundContext context, nfs_resop4 result)
		...
		res.resok4.obj_attributes = getAttributes(_args.opgetattr.attr_request,
				context.getFs(), context.currentInode(), context);
		res.status = nfsstat.NFS_OK;

	}

	static fattr4 getAttributes(bitmap4 bitmap, VirtualFileSystem fs,
			Inode inode, Stat stat, CompoundContext context)
			throws IOException, OncRpcException {
		/*
		...
		for (int i : bitmap) {
			Optional<XdrAble> optionalAttr = (Optional<XdrAble>) fattr2xdr(i,
					fs, inode, stat, context);
			if (optionalAttr.isPresent()) {
				XdrAble attr = optionalAttr.get();
				_log.debug("   getAttributes : {} ({}) OK.", i,
						attrMask2String(i));
				attr.xdrEncode(xdr);
				processedAttributes.set(i);
			} else {
				_log.debug("   getAttributes : {} ({}) NOT SUPPORTED.", i,
						attrMask2String(i));
			}
		}
	...
	static fattr4 getAttributes(bitmap4 bitmap, VirtualFileSystem fs,
			Inode inode, CompoundContext context) throws IOException,
			OncRpcException {
		return getAttributes(bitmap, fs, inode, context.getFs().getattr(inode),
				context);
	}

	private static FsStat getFsStat(FsStat fsStat, VirtualFileSystem fs)
			throws IOException {
		if (fsStat != null) {
			return fsStat;
		}
	...
	/**
	 * get inodes requested attribute and converted into RPC xdr format operates
	 * with READ and R/W attributes
	 * 
	 * @param fattr
	 * @param inode
	...
	// read/read-write
	private static Optional<? extends XdrAble> fattr2xdr(int fattr,
			VirtualFileSystem fs, Inode inode, Stat stat,
		...
		case nfs4_prot.FATTR4_FH_EXPIRE_TYPE:
			fattr4_fh_expire_type fh_expire_type = new fattr4_fh_expire_type(
					nfs4_prot.FH4_PERSISTENT);
		...
		case nfs4_prot.FATTR4_SYMLINK_SUPPORT:
			fattr4_symlink_support symlink_support = new fattr4_symlink_support(
					true);
		...
		case nfs4_prot.FATTR4_LEASE_TIME:
			return Optional.of(new fattr4_lease_time(
		...
		case nfs4_prot.FATTR4_ACLSUPPORT:
			fattr4_aclsupport aclSupport = new fattr4_aclsupport(
					nfs4_prot.ACL4_SUPPORT_ALLOW_ACL
		...
		case nfs4_prot.FATTR4_FILES_AVAIL:
			...
			fattr4_files_avail files_avail = new fattr4_files_avail(
					fsStat.getTotalFiles() - fsStat.getUsedFiles());
			return Optional.of(files_avail);
		case nfs4_prot.FATTR4_FILES_FREE:
			fsStat = getFsStat(fsStat, fs);
			fattr4_files_free files_free = new fattr4_files_free(
					fsStat.getTotalFiles() - fsStat.getUsedFiles());
		...
		case nfs4_prot.FATTR4_MAXFILESIZE:
			return Optional.of(new fattr4_maxfilesize(
					NFSv4Defaults.NFS4_MAXFILESIZE));
		...
		case nfs4_prot.FATTR4_MAXNAME:
			return Optional.of(new fattr4_maxname(
					NFSv4Defaults.NFS4_MAXFILENAME));
		case nfs4_prot.FATTR4_MAXREAD:
			return Optional.of(new fattr4_maxread(
					NFSv4Defaults.NFS4_MAXIOBUFFERSIZE));
		case nfs4_prot.FATTR4_MAXWRITE:
			fattr4_maxwrite maxwrite = new fattr4_maxwrite(
					NFSv4Defaults.NFS4_MAXIOBUFFERSIZE);
		...
		case nfs4_prot.FATTR4_SPACE_AVAIL:
			fsStat = getFsStat(fsStat, fs);
			fattr4_space_avail spaceAvail = new fattr4_space_avail(
					fsStat.getTotalSpace() - fsStat.getUsedSpace());
			return Optional.of(spaceAvail);
		case nfs4_prot.FATTR4_SPACE_FREE:
			fsStat = getFsStat(fsStat, fs);
			fattr4_space_free space_free = new fattr4_space_free(
					fsStat.getTotalSpace() - fsStat.getUsedSpace());
		...
		case nfs4_prot.FATTR4_MOUNTED_ON_FILEID:
			...
			fattr4_mounted_on_fileid mounted_on_fileid = new fattr4_mounted_on_fileid(
					mofi);
