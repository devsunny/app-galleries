...
public class JavaRestControllerGenerator extends CodeGenerator {
	...
	protected void doGroupBy(StringBuilder methods) throws IOException {
		...
		methods.append(String
				.format("%1$s@RequestMapping(value=\"%2$s\" method = { RequestMethod.GET })\n",
						INDENDENT_2, uri.toString()));
		methods.append(INDENDENT_2).append("@ResponseBody\n");
		if (gbFields.size() == 1) {
			Field keyField = gbFields.get(0);
			methods.append(String
					.format("%2$spublic java.util.List<%1$s> select%1$sGroupBy%3$s(%5$s %4$s){\n",
							entity.getEntityObjectName(), INDENDENT_2,
							keyField.getObjectname(), keyField.getVarname(),
							JdbcSqlTypeMap.toJavaTypeName(keyField)));
			methods.append(String
					.format("%2$s return this.%6$sMapper.select%1$sGroupBy%3$s(%4$s);\n",
							entity.getEntityObjectName(), INDENDENT_2,
							keyField.getObjectname(), keyField.getVarname(),
							JdbcSqlTypeMap.toJavaTypeName(keyField),
							entity.getEntityVarName()));
			...
			methods.append(String
					.format("%1$spublic java.util.List<%2$s> select%2$sGroupBy%4$s(%2$s %3$s){\n",
							INDENDENT_2, entity.getEntityObjectName(),
							entity.getEntityVarName(), buf.toString()));

			methods.append(String
					.format("%1$s return this.%3$sMapper.select%2$sGroupBy%4$s(%3$s){\n",
							INDENDENT_2, entity.getEntityObjectName(),
							entity.getEntityVarName(), buf.toString()));
	...
	protected void doDrillDown(StringBuilder methods) throws IOException {
		...
		for (int i = 0; i < ddFields.size(); i++) {
			...
			for (int k = 0; k <= i; k++) {
				...
				params.add(String.format("@PathVariable(\"%2$s\")%1$s %2$s",
						JdbcSqlTypeMap.toJavaTypeName(ddk), ddk.getVarname()));
			}
			...
			methods.append(INDENDENT_2).append("@ResponseBody\n");
			methods.append(String
					.format("%1$s public java.util.List<%2$s> select%2$sDrillDownBy%4$s(%5$s){\n",
							INDENDENT_2, entity.getEntityObjectName(),
			...
			for (int k = 0; k <= i; k++) {
				...
				methods.append(String.format("%1$s %3$s.set%4$s(%5$s);\n",
						INDENDENT_2, entity.getEntityObjectName(),
						entity.getEntityVarName(), ddk.getObjectname(),
						ddk.getVarname()));
			}
			methods.append(String
					.format("%1$s return this.%3$sMapper.select%2$sDrillDownBy%4$s(%3$s);\n",
							INDENDENT_2, entity.getEntityObjectName(),
							entity.getEntityVarName(), dd0.getObjectname()));
